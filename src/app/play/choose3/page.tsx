...
export default function Choose3GamePage() {
  return (
...
function Choose3Game() {
  ...
  const runSequence = useCallback(async (letter: Letter, sourceLetters: Letter[]) => {
    ...
    const wrong = getRandomWrongLetters(letter.letter, 2, choicePool);
    ...
    initDB().then(async () => {
      ...
      if (isAdaptive) {
        ...
        if (startParam !== null) {
          ...
          const gs = await getGameState('choose3');
  ...
  async function advanceToNextChoose3() {
    ...
    if (nextIndex === 0) {
      ...
      if (newCompletions >= 3) {
        await saveGameState('choose3', 0, newCompletions);
  ...
  async function handleChoice(choice: string) {
    ...
    if (choice === current.letter) {
      await recordLetterAttempt(childName, current.letter, true, responseTime, 'choose3');
      ...
      setPhase('wrong');
      await recordLetterAttempt(childName, current.letter, false, 0, 'choose3');
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen p-6 gap-4 relative">
      ...
      {phase === 'hidden' && (<div className="text-6xl text-gray-300 animate-pulse">...</div>)}
          ...
          <div className="flex gap-4">
            {choices.map((ch) => (
              <button key={ch} onClick={() => handleChoice(ch)}
                className="w-28 h-28 md:w-36 md:h-36 bg-blue-500 hover:bg-blue-600 active:bg-blue-700 rounded-3xl shadow-2xl flex items-center justify-center transform transition-all hover:scale-105 active:scale-95">
                <span className="text-5xl md:text-6xl font-extrabold text-white">{ch}</span>
              </button>
            ))}
          </div>
        </div>
