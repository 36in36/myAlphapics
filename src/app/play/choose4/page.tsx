...
export default function Choose4GamePage() {
  return (
...
function Choose4Game() {
  ...
  const runSequence = useCallback(async (letter: Letter, sourceLetters: Letter[]) => {
    ...
    const wrong = getRandomWrongLetters(letter.letter, 3, choicePool);
    ...
    initDB().then(async () => {
      ...
      if (isAdaptive) {
        ...
        if (startParam !== null) {
          ...
          const gs = await getGameState('choose4');
  ...
  async function advanceToNextChoose4() {
    ...
    if (nextIndex === 0) {
      ...
      if (newCompletions >= 3) {
        await saveGameState('choose4', 0, newCompletions);
    ...
    if (!adaptiveRef.current) {
      await saveGameState('choose4', nextIndex, newCompletions);
    }
    setIndex(nextIndex);
    setStep((s) => s + 1);
  }
  ...
  async function handleChoice(choice: string) {
    ...
    if (choice === current.letter) {
      ...
      setPhase('wrong');
      await recordLetterAttempt(childName, current.letter, false, 0, 'choose4');
